...
foreach ($sourceImages as $imagem) {
    ...
    if (count($matches) >= 4) {
        ...
        foreach ($result as $row) {
            $codigoBanco = $row["COD_SEQUENCIAL"];
            $modeloBanco = $row["MODELO"];
            ...
            } elseif ($codigoBanco == $codigoImagem) {
                $codigoCorreto = $codigoBanco;
                $modeloCorreto = $modeloBanco;
            }
        }
        ...
        if (!$imagemCorreta) {
            ...
            echo '<div>';
            echo '<span class="font-medium">Nome de imagem incorreto!</span> A imagem \'' . $imagem . '\' está com nome incorreto.<br>';
            echo $codigoImagem .' e '. $modeloImagem;
            echo '<br>';
            if (!empty($codigoCorreto)) {
                echo 'Código sequencial correto: ' . $codigoCorreto . '<br>';
            }
            if (!empty($modeloCorreto)) {
                echo 'Código de modelo correto: ' . $modeloCorreto . '<br>';
            }
            echo '</div>';
            echo '</div>';
        }
