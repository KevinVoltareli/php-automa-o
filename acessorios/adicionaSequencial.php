...
$sourceDir = '\\\\192.168.30.200\\dados$\\Depto E-Commerce\\teste\\renameSeq\\';
...
try {
    ...
    foreach ($sourceImages as $imagem) {
        preg_match('/^(.*?)_(.*?)_/', $imagem, $matches);

        if (count($matches) >= 3) {
            $codigoImagem = $matches[2];

            foreach ($result as $row) {
                $codigoBanco = $row["FANTASIA"];
                ...
                if (strpos($codigoBanco, $codigoImagem) !== false) {
                    ...
                    $novoNomeImagem = "$codSequencial" . "_" . $imagem;
